...
class CatBoostModel(BaseModel):
    ...
    def create_features(self, df: pd.DataFrame, target_col: str) -> Tuple[np.ndarray, np.ndarray]:
        """Использовать все фичи кроме даты и целевой переменной"""
    ...
    def _cyclic_time_features(self, df):
        # Добавляет циклические признаки для месяца, дня недели, дня года
    ...
    def _update_dynamic_features(self, df, target_col):
        # Рекурсивно обновляет rolling/diff/pct_change для всех динамических признаков
    ...
    def plot_test_predictions(self) -> go.Figure:
        """Plot predictions vs actuals for the last test fold after cross-validation"""
        ...
        fig.update_layout(
            title='CatBoost: прогноз vs факт (последний фолд)',
            xaxis_title='Дата',
